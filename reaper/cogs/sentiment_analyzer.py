...
class SentimentAnalyzer(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if not message.content:
            return
        if message.channel.id not in globals.config["channels"]["sentiment-analysis-channels"]:
            return
        ...
        try:
            result = await asyncio.to_thread(self.sentiment, message.content) # need to do this otherwise the bot will not do anything until this is done
        ...
        total_normal = self.bot.cached_sentiment[message.channel.id]["neutral"] + self.bot.cached_sentiment[message.channel.id]["positive"]
        ...
        if total >= messages_to_cache:
            if total_negative > total_normal:
                ...
                await moderator_channel.send(f"Negative sentiment detected in {message.channel.mention}! Out of the last {messages_to_cache} messages, {total_negative} were negative.")
            self.reset_message_cache()
